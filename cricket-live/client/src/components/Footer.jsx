        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(180px, 1fr))", gap: 32, marginBottom: 32 }}>
          <div>
            <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 12 }}>
              ...
              <span style={{ fontWeight: 800, fontSize: 18, color: "var(--green)" }}>CricLive</span>
            </div>
          ...
          <div>
            ...
            {[["News", "/news"], ["Videos", "/videos"], ["Stats", "/stats"], ["About", "/about"]].map(([l, h]) => (
              <Link key={h} to={h} style={{ display: "block", color: "var(--text2)", fontSize: 13, marginBottom: 8 }}>{l}</Link>
            ))}
          </div>
        </div>
        <div style={{ borderTop: "1px solid var(--border)", paddingTop: 20, textAlign: "center", fontSize: 12, color: "var(--text3)" }}>
          © 2026 CricLive. Data powered by <a href="https://cricketdata.org" target="_blank" rel="noreferrer" style={{ color: "var(--green)" }}>CricketData.org</a> & Cricbuzz via RapidAPI.
        </div>
      </div>
    </footer>
  );
